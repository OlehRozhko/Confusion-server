...
.put(cors.corsWithOptions, verifyUser, (req, res, next) => {
  ...
  .then((dish) => {
    if((req.user._id).equals(dish.comments.id(req.params.commentId).author)){
      ...
      let err = new Error('You cannot do update this comment, because you are not the author');
      err.status = 403;
      return next(err);
    }
  }, err => next(err))
...
.delete(cors.corsWithOptions, verifyUser, (req, res, next) => {
  ...
  .then((dish) => {
    if((req.user._id).equals(dish.comments.id(req.params.commentId).author)){
      ...
      let err = new Error('You cannot do delete this comment, because you are not the author');
      err.status = 403;
      return next(err);
    }
  }, (err) => next(err))
